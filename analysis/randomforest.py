...
def run_random_forest(df):
    ...
    with sns.plotting_context(font_scale=axis_scale):
        #ax = sns.barplot(y=X.columns, x=importances)
        ax = sns.heatmap(X, xticklabels=labels_abc9_ops, yticklabels=y_axis, cmap='coolwarm')
        ...
        plt.savefig(df.loc[0, "Benchmark"]+"_HistoryPassImportance.png", format="png", dpi=300)
        plt.close
