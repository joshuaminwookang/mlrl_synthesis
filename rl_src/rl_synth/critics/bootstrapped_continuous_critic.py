...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        # print("ob:" + str(ob_no.shape))
        # print("next_ob:" + str(next_ob_no.shape))
        # print("reward:" + str(reward_n.shape))
        # print("terminal:" + str(terminal_n.shape))
        ob_no = ptu.from_numpy(ob_no)
        next_ob_no = ptu.from_numpy(next_ob_no)
        reward_n = ptu.from_numpy(reward_n.squeeze(1))
        terminal_n = ptu.from_numpy(terminal_n)
        loss = 0
        for _ in range(self.num_target_updates):
            # calculate V(s') and target 
            value_next = self.critic_network(next_ob_no).squeeze(1)
            target = reward_n + self.gamma * value_next * (1-terminal_n)
            target = target.detach()
            
            for _ in range(self.num_grad_steps_per_target_update):
            # calculate V(s) and fit value function
                value = self.critic_network(ob_no).squeeze(1)
