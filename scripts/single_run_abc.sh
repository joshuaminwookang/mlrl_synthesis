...
#YOSYS=${YOSYS:-/home/arya/src/yosys/yosys}
if [[ "${synth}" == *"yosys"* ]] && [ ! -x "${YOSYS}" ]; then
  echo "abc required for synth but executable not usable: ${ABC}"
  exit 10
fi
...
EOT
  ...
  if [ -f "${edif}" ]; then
      ...
      # fi
      echo "# auto-generated by single_run_abc.sh" > $ip.ys
      ...
      # TODO: convert Verilog to BLIF using Yosys
      cat >> ${ip}.ys <<EOT
read_blif internal.blif 
write_edif ${edif}
#read_verilog ${mem_file}
#write_blif ${pwd}/${ip}_syn.v
#write_blif ${pwd}/${ip}_syn.v
...
# synth_xilinx -flatten ${synth_with_abc9} -edif ${edif}
      ...
      echo -e "read $path; &get; source ${ip}.${2}.abc.script;strash;write internal.blif" | ${ABC}
